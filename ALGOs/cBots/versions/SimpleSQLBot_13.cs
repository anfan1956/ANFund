    ...
    public class SimpleSQLBot : Robot
    {
        ...
        private static string GetTradingPositions(Position[] positions)
        {
            ...
            for (int i = 0; i < positions.Length; i++)
            {
                ...
                string openTime = position.EntryTime.ToString("yyyy-MM-dd:HH:mm:ss"); // формат с двоеточием!

                // Получаем UUID из Label
                if (!(string.IsNullOrEmpty(position.Label) || !Guid.TryParse(position.Label, out Guid positionUuid)))
                {
                    sqlValues.Append($"('{positionUuid}',");  // positionLabel
                }
                else
                {
                    sqlValues.Append($"('{Guid.NewGuid()}',");  // новый UUID
                }
                ...
                sqlValues.Append($"'{openTime}')");                   // OpenTime
        ...
        // Вспомогательный метод для получения ID позиции из БД
        private static string GetPositionDbId(long cTraderPositionId)
        {
            // Здесь нужно получить ID из trd.position по positionTicket
            // Пока заглушка - нужно реализовать запрос к БД
            return $"(SELECT ID FROM trd.position WHERE positionTicket = {cTraderPositionId})";
        }

        // Конвертация TimeFrame в строку
        private static string GetTimeFrameString(TimeFrame timeFrame)
        {
        ...
        private string GetSqlScript(Position[] positions)
        {
            ...
            {
                var position = positions[i];
                string openTime = position.EntryTime.ToString("yyyy-MM-dd:HH:mm:ss"); // ВАЖНО: формат с двоеточием!

                // Получаем UUID из Label позиции
                if (!string.IsNullOrEmpty(position.Label) && Guid.TryParse(position.Label, out Guid positionUuid))
                {
                    // Есть UUID в Label
                    sqlValues.Append($"('{positionUuid}',");  // positionLabel
                ...
                {
                    // Нет UUID - генерируем новый или используем пустой
                    sqlValues.Append($"('{Guid.NewGuid()}',");  // генерируем новый UUID
                }
        ...
        declare @broker VARCHAR(MAX) = '{broker.Replace("'", "''")}'
            ...
            , @platformVersion NVARCHAR(20) = '{platformVersion}';
        ...
        exec trd.positions_p @positions, @broker, @account, @platformCode, @platformVersion;";
            ...
            Print("=== SQL SCRIPT FOR trd.positions_p ===");
            Print(sqlScript);
            Print("======================================");
