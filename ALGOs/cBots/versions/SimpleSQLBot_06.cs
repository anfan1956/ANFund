    ...
    public class SimpleSQLBot : Robot
    {
                       ...
                       double stopLossPercent = -10, double takeProfitPercent = 20)
        {
            try
            {
                ...
                double? stopLossPrice = price * (1 + (stopLossPercent / 100));
                double? takeProfitPrice = price * (1 + (takeProfitPercent / 100));

                Print($"=== PLACING ORDER ===");
                Print($"Symbol: {symbol}");
                Print($"Current Price: {currentPrice.ToString(PRICE_FORMAT)}");
                Print($"Order Price: {price.ToString(PRICE_FORMAT)}");
                Print($"Volume: {volume.ToString(VOLUME_FORMAT)} lots");
                Print($"Stop Loss: {stopLossPrice?.ToString(PRICE_FORMAT)} ({stopLossPercent.ToString(PERCENT_FORMAT)}%)");
                Print($"Take Profit: {takeProfitPrice?.ToString(PRICE_FORMAT)} ({takeProfitPercent.ToString(PERCENT_FORMAT)}%)");
                ...
                if (priceDifference < minDifference)
                {
                    ...
                    // MODERN API - Using proper parameter names
                    result = ExecuteMarketOrder(
                        tradeType: TradeType.Buy,
                        symbolName: symbol,
                        volumeInUnits: symbolObj.QuantityToVolumeInUnits(volume),
                        label: $"Market_{symbol}",
        ...
        private new TradeResult PlaceStopOrder(TradeType tradeType, string symbolName, double volumeInUnits, double targetPrice, string label, double? stopLoss, double? takeProfit)
        {
            throw new NotImplementedException();
        }

        private new TradeResult PlaceLimitOrder(TradeType tradeType, string symbolName, double volumeInUnits, double targetPrice, string label, double? stopLoss, double? takeProfit)
        {
            throw new NotImplementedException();
        }

        private new TradeResult ExecuteMarketOrder(TradeType tradeType, string symbolName, double volumeInUnits, string label, double? stopLoss, double? takeProfit)
        {
            throw new NotImplementedException();
        }
