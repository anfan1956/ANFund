...
namespace SimpleSQLBot
{
    ...
    public class SimpleSQLBot : Robot
    {
        ...
        // В начале класса SimpleSQLBot, после _symbolTickerJids - for processing all timeframes
        private readonly Dictionary<string, Dictionary<int, DateTime>> _lastProcessedBarTimes = new();
        private readonly TimeFrame[] _timeFramesToCollect = new TimeFrame[]
        {
            TimeFrame.Minute,       // M1
        ...
        protected override void OnStart()
        {
            ...
            Print($"Server.Time (UTC): {Server.Time:yyyy-MM-dd HH:mm:ss}");
            Print($"Server.Time.ToLocalTime(): {Server.Time.ToLocalTime():yyyy-MM-dd HH:mm:ss}");
            Print($"DateTime.Now: {DateTime.Now:yyyy-MM-dd HH:mm:ss}");
            Print($"DateTime.UtcNow: {DateTime.UtcNow:yyyy-MM-dd HH:mm:ss}");
            Print($"=== END TIME DEBUG ===");
            ...
            // После загрузки symbol mappings добавь:
            Print("=== CHECKING POSITIONSTATE TABLE ===");
            try
            {
                using SqlCommand checkCmd = new SqlCommand("SELECT TOP 1 ID, positionID, timestamp FROM trd.positionState ORDER BY timestamp DESC", _connectionManager.GetConnection());
                using SqlDataReader reader = checkCmd.ExecuteReader();
                if (reader.Read())
                {
                    Print($"Last record in positionState: ID={reader[0]}, PositionID={reader[1]}, Time={reader[2]}");
                }
                else
                {
                    Print("No records in positionState table");
                }
            }
            catch (Exception ex)
            {
                Print($"Error checking positionState: {ex.Message}");
            }
        ...
        // Заменяем текущий метод OnBar() на этот:

        protected override void OnBar()
        {
            Print($"New Bar Started for timeframe: {TimeFrame}");
            try
            {
                DateTime currentBarTime = Server.Time.ToLocalTime();
                ...
                if (currentBarTime > _lastBarTime)
                {
                    ...
                    Print($"DEBUG: currentBarTime (Local): {currentBarTime:yyyy-MM-dd HH:mm:ss}");
                    Print($"DEBUG: Server.Time (UTC): {Server.Time:yyyy-MM-dd HH:mm:ss}");
        ...
        // Добавляем новый метод для обновления состояния позиций:
        private void UpdatePositionStates(Position[] positions)
        {
            try
            {
                ...
                Print($"Updating state for {positions.Length} positions...");

                // Создаем DataTable для trd.PositionStateTableType
                DataTable positionStatesTable = new ();
                positionStatesTable.Columns.Add("positionID", typeof(int));
                positionStatesTable.Columns.Add("timestamp", typeof(DateTime));
                positionStatesTable.Columns.Add("currentPrice", typeof(decimal));
                positionStatesTable.Columns.Add("commission", typeof(decimal));
                positionStatesTable.Columns.Add("swap", typeof(decimal));
                positionStatesTable.Columns.Add("stopLoss", typeof(decimal));
                positionStatesTable.Columns.Add("takeProfit", typeof(decimal));
                positionStatesTable.Columns.Add("netProfit", typeof(decimal));
                positionStatesTable.Columns.Add("grossProfit", typeof(decimal));

                foreach (var position in positions)
                {
                    DataRow row = positionStatesTable.NewRow();
                    row["positionID"] = position.Id;
                    row["timestamp"] = Server.Time; // Используем текущее время сервера
                    row["currentPrice"] = Convert.ToDecimal(position.CurrentPrice);
                    row["commission"] = Convert.ToDecimal(position.Commissions);
                    row["swap"] = Convert.ToDecimal(position.Swap);
                    row["stopLoss"] = position.StopLoss.HasValue ?
                        Convert.ToDecimal(position.StopLoss.Value) : (object)DBNull.Value;
                    row["takeProfit"] = position.TakeProfit.HasValue ?
                        Convert.ToDecimal(position.TakeProfit.Value) : (object)DBNull.Value;
                    row["netProfit"] = Convert.ToDecimal(position.NetProfit);
                    row["grossProfit"] = Convert.ToDecimal(position.GrossProfit);

                    positionStatesTable.Rows.Add(row);

                    Print($"  Position {position.Id}: Price={position.CurrentPrice:F5}, Net={position.NetProfit:F2}");
                }

                // Вызываем хранимую процедуру
                using SqlCommand cmd = new ("trd.sp_UpdatePositionStates", _connectionManager.GetConnection());
                cmd.CommandType = System.Data.CommandType.StoredProcedure;

                SqlParameter param = cmd.Parameters.AddWithValue("@positionStates", positionStatesTable);
                param.SqlDbType = SqlDbType.Structured;
                param.TypeName = "trd.PositionStateTableType";

                int rowsAffected = cmd.ExecuteNonQuery();
                Print($"Position states updated: {rowsAffected} rows affected");
            }
            catch (Exception ex)
            {
                Print($"Error in UpdatePositionStates: {ex.Message}");
            }
        }

        private int ProcessBarsForTimeFrame(Symbol symbol, TimeFrame timeFrame)
        {
            int barsProcessed = 0;
            ...
            try
            {
                int timeFrameId = GetTimeFrameId(timeFrame);

                // Получаем время последнего обработанного бара для этого символа и ТФ
                DateTime lastProcessedTime = GetLastProcessedTime(symbol.Name, timeFrameId);

                Print($"    DEBUG: Processing {symbol.Name} {timeFrame}");
                Print($"    DEBUG: Last processed time from memory/DB: {lastProcessedTime:yyyy-MM-dd HH:mm:ss}");

                // Получаем бары для символа и таймфрейма
                var bars = MarketData.GetBars(timeFrame, symbol.Name);

                Print($"    DEBUG: Total bars available: {bars.Count}");
                if (bars.Count > 0)
                {
                    Print($"    DEBUG: First bar: {bars[0].OpenTime:yyyy-MM-dd HH:mm:ss}");
                    Print($"    DEBUG: Last bar: {bars[^1].OpenTime:yyyy-MM-dd HH:mm:ss}");
                }

                if (bars.Count < 2)
                {
                    Print($"    DEBUG: Not enough bars (< 2), skipping");
                    return 0;
                }
                ...
                Print($"    DEBUG: Starting from index {startIndex} (bar time: {bars[startIndex].OpenTime:yyyy-MM-dd HH:mm:ss})");
                ...
                for (int i = startIndex; i >= 0; i--)
                {
                    ...
                    {
                        Print($"    DEBUG: Found NEW bar at index {i}: {bar.OpenTime:yyyy-MM-dd HH:mm:ss}");
                    ...
                    {
                        Print($"    DEBUG: Bar {bar.OpenTime:yyyy-MM-dd HH:mm:ss} is OLDER than last processed {lastProcessedTime:yyyy-MM-dd HH:mm:ss}, stopping");
                        break; // Дошли до уже обработанных баров
                    }
                }
                ...
                if (newBars.Count > 0)
                {
                    ...
                    Print($"    DEBUG: Sending {newBars.Count} bars to SQL");
                    SendBarsToSql(newBars);

                    // Обновляем время последнего обработанного бара
                    var lastBar = newBars[^1];
                    UpdateLastProcessedTime(symbol.Name, timeFrameId, lastBar.BarTime);

                    Print($"    DEBUG: Updated last processed time to: {lastBar.BarTime:yyyy-MM-dd HH:mm:ss}");
                }
                else
                {
                    Print($"    DEBUG: No new bars found since {lastProcessedTime:yyyy-MM-dd HH:mm:ss}");
                }
            }
            catch (Exception ex)
            {
                Print($"Error processing bars for {symbol.Name} ({timeFrame}): {ex.Message}");
            }

            Print($"    DEBUG: Total bars processed: {barsProcessed}");
        ...
        //                        : 0,
        //                    BarTime = bar.OpenTime,
        //                    TimeFrameID = GetTimeFrameId(TimeFrame),
        //                    OpenValue = bar.Open,
        //                    CloseValue = bar.Close,
        //                    HighValue = bar.High,
        //                    LowValue = bar.Low,
        ...
        private int ProcessHistoricalBarsForSymbol(Symbol symbol, TimeFrame timeFrame, List<Bar> historicalBars)
        {
            int barsProcessed = 0;
            List<BarData> barsToSend = new();
            ...
            try
            {
                ...
                foreach (var bar in historicalBars)
                {
                    ...
                    {
                        TickerJID = _symbolTickerJids[symbol.Name],
                        BarTime = bar.OpenTime.ToLocalTime(), // Конвертируем UTC → Local
                        TimeFrameID = timeFrameId,
                        OpenValue = bar.Open,
                        CloseValue = bar.Close,
                        HighValue = bar.High,
                ...
                if (barsToSend.Count > 0)
                {
                    ...
                    // Обновляем время последнего обработанного бара
                    var lastBarTime = historicalBars[^1].OpenTime;
                    UpdateLastProcessedTime(symbol.Name, timeFrameId, lastBarTime);
                }
            }
            catch (Exception ex)
            {
                Print($"Error processing historical bars for {symbol.Name} ({timeFrame}): {ex.Message}");
            }
        ...
        private void CollectBarsData()
        {
            ...
            try
            {
                Print($"=== COLLECTING BARS DATA (ALL TIMEFRAMES) ===");
                ...
                foreach (var symbolName in symbols)
                {
                    ...
                    foreach (var timeFrame in _timeFramesToCollect)
                    {
                        try
                        {
                            int barsProcessed = ProcessBarsForTimeFrame(symbol, timeFrame);
                            totalBarsProcessed += barsProcessed;

                            if (barsProcessed > 0)
                            {
                                Print($"    {timeFrame}: {barsProcessed} new bars");
                            }
                        }
                        catch (Exception ex)
                        {
                            Print($"    ERROR processing {timeFrame}: {ex.Message}");
                        }
                    }
                }
        ...
        private void LoadHistoricalBars()
        {
            ...
            try
            {
                ...
                foreach (var symbolName in symbols)
                {
                    ...
                    // Загружаем исторические данные по ВСЕМ таймфреймам
                    foreach (var timeFrame in _timeFramesToCollect)
                    {
                        try
                        {
                            ...
                            int timeFrameId = GetTimeFrameId(timeFrame);
                            DateTime? lastBarTimeInDb = GetLastBarTimeFromDatabase(symbol.Name, timeFrameId);

                            DateTime startDate = requestedStartDate;
                            if (lastBarTimeInDb.HasValue && lastBarTimeInDb.Value > requestedStartDate)
                            {
                                startDate = lastBarTimeInDb.Value.AddMinutes(1);
                            ...
                            // Получаем исторические бары
                            var historicalBars = GetHistoricalBars(symbol, timeFrame, startDate, requestedEndDate);

                            if (historicalBars != null && historicalBars.Count > 0)
                            {
                                int barsProcessed = ProcessHistoricalBarsForSymbol(symbol, timeFrame, historicalBars);
                                totalHistoricalBars += barsProcessed;
                                Print($"    {timeFrame}: Loaded {barsProcessed} historical bars");
                            }
                        }
                        catch (Exception ex)
                        {
                            Print($"    ERROR loading {timeFrame} data: {ex.Message}");
                        }
                    }
                }
        ...
        private DateTime? GetLastBarTimeFromDatabase(string symbolName, int timeFrameId = -1)
        {
            try
            {
                ...
                string sql = @"SELECT MAX(barTime) FROM tms.Bars 
                       WHERE TickerJID = @tickerJID";

                var parameters = new List<SqlParameter>
        {
            new("@tickerJID", tickerJID)
        };

                // Если указан конкретный ТФ, фильтруем по нему
                if (timeFrameId > 0)
                {
                    sql += " AND timeframeID = @timeframeID";
                    parameters.Add(new SqlParameter("@timeframeID", timeFrameId));
                }
                else
                {
                    // Если ТФ не указан, получаем последний бар по любому ТФ
                    sql += " AND timeframeID = @timeframeID";
                    parameters.Add(new SqlParameter("@timeframeID", GetTimeFrameId(TimeFrame.Minute)));
                }

                using SqlCommand cmd = new(sql, _connectionManager.GetConnection());
        ...
        private List<Bar> GetHistoricalBars(Symbol symbol, TimeFrame timeFrame, DateTime startDate, DateTime endDate)
        {
            ...
            try
            {
                ...
                // ВРЕМЕННАЯ ОТЛАДКА ДЛЯ ВРЕМЕНИ БАРОВ
                Print($"    === TIME DEBUG for {symbol.Name} ({timeFrame}) ===");
                if (historicalSeries.Count > 0)
                {
                    var firstBar = historicalSeries.First();
                    var lastBar = historicalSeries.Last();

                    Print($"    First bar OpenTime: {firstBar.OpenTime:yyyy-MM-dd HH:mm:ss}");
                    Print($"    First bar OpenTime Kind: {firstBar.OpenTime.Kind}");
                    Print($"    Last bar OpenTime: {lastBar.OpenTime:yyyy-MM-dd HH:mm:ss}");
                    Print($"    Last bar OpenTime Kind: {lastBar.OpenTime.Kind}");
                    Print($"    Current Server.Time: {Server.Time:yyyy-MM-dd HH:mm:ss}");
                    Print($"    Current Server.Time.ToLocalTime(): {Server.Time.ToLocalTime():yyyy-MM-dd HH:mm:ss}");
                    Print($"    DateTime.UtcNow: {DateTime.UtcNow:yyyy-MM-dd HH:mm:ss}");
                    Print($"    DateTime.Now: {DateTime.Now:yyyy-MM-dd HH:mm:ss}");
                    Print($"    === END TIME DEBUG ===");
                }
        ...
        private void InitializeBarCollection()
        {
            try
            {
                ...
                Print($"  TimeFrame: {TimeFrame} (ID: {GetTimeFrameId(TimeFrame)})");
        ...
        private DateTime GetLastProcessedTime(string symbol, int timeFrameId)
        {
            try
            {
                if (_lastProcessedBarTimes.ContainsKey(symbol) &&
                    _lastProcessedBarTimes[symbol].ContainsKey(timeFrameId))
                {
                    return _lastProcessedBarTimes[symbol][timeFrameId];
                }

                // Если нет в памяти, пытаемся получить из БД
                return GetLastBarTimeFromDatabase(symbol, timeFrameId) ?? DateTime.MinValue;
            }
            catch
            {
        ...
        private void UpdateLastProcessedTime(string symbol, int timeFrameId, DateTime time)
        {
            if (!_lastProcessedBarTimes.ContainsKey(symbol))
            {
                _lastProcessedBarTimes[symbol] = new Dictionary<int, DateTime>();
            }

            _lastProcessedBarTimes[symbol][timeFrameId] = time;
        }
