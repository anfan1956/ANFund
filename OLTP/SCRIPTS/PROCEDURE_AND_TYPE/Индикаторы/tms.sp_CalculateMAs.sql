use cTrader
go
--D:\TradingSystems\OLTP\OLTP\PROCEDURE_AND_TYPE\Индикаторы\tms.sp_CalculateMAs.sql

if OBJECT_ID ('tms.sp_CalculateMAs') is not null drop proc tms.sp_CalculateMAs
go

create proc tms.sp_CalculateMAs 
	@timeGap INT = null, 
...
Begin
	SET NOCOUNT ON;
	SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; -- NO Blocks

	DECLARE @ErrorMsg NVARCHAR(4000);
	DECLARE @cutOffTime  DATETIME  = DATEADD(MINUTE, -@TimeGap,  (SELECT max(bartime) FROM tms.bars));;

	BEGIN TRY;
		if @timeGap is null truncate table tms.ma;	
...
;
	...
	PRINT 'MA calculation completed. Rows processed: ' + CAST(@RowsProcessed AS VARCHAR);
	RETURN @RowsProcessed;
	END TRY
	BEGIN CATCH
		SET @ErrorMsg = ERROR_MESSAGE();
		SET @RowsProcessed = -1; -- код ошибки
		
		PRINT 'ERROR in sp_CalculateMAs: ' + @ErrorMsg;
...
GO
--
declare @timeGap int = 120;
exec tms.sp_CalculateMAs @timeGap = @timeGap

--select count (*) from tms.ma 
