...
class MTFRSIEMAStrategy:
    ...
    def __init__(self, configuration_id: int):
        ...
        try:
            ...
            init_conn = pyodbc.connect(self.connection_string, autocommit=True)
            try:
    ...
    def _get_connection_string(self):
        ...
        Using standard connection string from working strategy
        """
        return (
    ...
    def get_open_positions(self, connection):
        ...
        try:
            ...
            row = cursor.fetchone()
            print(f"[DEBUG] Raw JSON from fn_GetStrategyPositionIDs: {row[0] if row and row[0] else 'None or empty'}")  # DEBUG
            ...
            if row and row[0]:
                try:
                    ...
                    print(f"[DEBUG] Parsed JSON type: {type(positions_data)}, data: {positions_data}")  # DEBUG

                    # Check if it's a list
                    if isinstance(positions_data, list):
                        for pos_data in positions_data:
                            print(f"[DEBUG] Position data item: {pos_data}")  # DEBUG
                            positions.append({
                                'id': pos_data.get('id'),
    ...
    def run(self):
        ...
        try:
            ...
            if not positions:
                ...
                position = self.open_initial_position(
                    ...
                    platform_id=self.platform_id,
                    strategy_configuration_id=self.configuration_id
                    # max_checks=self.max_position_checks,
        ...
        try:
            while True:
                ...
                if termination:
                    # print(f"\n[Termination] Strategy {self.configuration_id} termination requested at {termination['requested_at']}")
                    # print(f"[Termination] Termination ID: {termination['termination_id']}")

                    # Close all open positions
                    term_conn = pyodbc.connect(self.connection_string, autocommit=True)
                    try:
                ...
                if last_strategy_run is None or (now - last_strategy_run).total_seconds() >= self.strategy_interval_seconds:
                    try:
                        ...
                        strategy_conn = pyodbc.connect(self.connection_string, autocommit=True)
                        try:
                ...
                if last_heartbeat_sent is None or (now - last_heartbeat_sent).total_seconds() >= 60:
                    heartbeat_conn = pyodbc.connect(self.connection_string, autocommit=True)
                    try:
            ...
            final_conn = pyodbc.connect(self.connection_string, autocommit=True)
            try:
            ...
            print(f"\nUnexpected error: {e}")
            # Cannot update tracker state here as we don't have a connection
            raise
